...
**basis** — linear algebra with tensor abstractions
...
Tests include:
- Hand-written tests in `test/scicloj/basis/`
- Generated tests from notebooks (via `kind/test-last`) in `test/basis_book/`
...
- `test/scicloj/basis/` — hand-written tests
- `test/basis_book/*_generated_test.clj` — generated from notebooks via Clay GFM rendering

### Generated Test Workflow

Notebooks use `kind/test-last` to embed assertions. When rendered to GFM via Clay,
test files are generated in `test/basis_book/`. These are run by the standard test runner.

To regenerate tests after editing a notebook:
```clojure
(require '[dev :as dev] :reload)
(dev/make-gfm! "basis_book/<name>.clj")
```
...
- **Use `def ^:private` not `defn-` in notebooks**: `defn-` displays its var name (`#'basis_book...`) in rendered output. Use `(def ^:private name (fn [...] ...))` to suppress this noise.
