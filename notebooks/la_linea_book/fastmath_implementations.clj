...
(ns la-linea-book.fastmath-implementations
  (:require
   ;; La Linea:
...
(let [n-iter 200]
  (tc/dataset {:method ["fastmath lm" "la-linea SVD" "la-linea normal eq."]
...
(let [xvs (mapv vector gp-xs)
      n-iter 100]
  (tc/dataset {:method ["fastmath GP" "la-linea GP"]
...
(let [n-iter 2000]
  (tc/dataset {:method ["fastmath RBF" "la-linea RBF"]
...
(let [n-iter 1000]
  (tc/dataset {:method ["fastmath kriging" "la-linea kriging"]
...
(let [n-iter 5000]
  (tc/dataset {:method ["fastmath Mahalanobis" "la-linea Mahalanobis"]
...
(let [helmert-coding (contrast/helmert levels)
      n-iter 5000]
  (tc/dataset {:method ["fastmath mean-contrasts" "la-linea mean-contrasts"]
...
(let [n-iter 5000]
  (tc/dataset {:method ["fastmath savgol-filter" "la-linea savgol"]
...
(let [offsets [-2 -1 0 1 2]
      n-iter 5000]
  (tc/dataset {:method ["fastmath fd-coeffs" "la-linea fd-coeffs"]
...
(let [n-iter 5000]
  (tc/dataset {:method ["fastmath quadrature" "la-linea quadrature"]
...
(let [n-iter 5000]
  (tc/dataset {:method ["fastmath ince-C" "la-linea ince-C"]
